...
export default function App({ navigation, route }) {
    ...
    //   try {
    //     const response = await axios.get(`http://localhost:3000/data?id=${user.idUser}`);
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/imageStreaming4`);
        if (Array.isArray(response.data) && response.data.length > 0) {
    ...
    const fetchStories = async () => {
      try {
        const response = await axios.get('http://localhost:3000/Userstories');
